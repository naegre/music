...
class LLM(torch.nn.Module):
    ...
    def inference(
            ...
            prob_dump_path: str = None,
            prob_top_k: int = 100,
            save_full_prob: bool = False,
        ...
        for i in range(int(max_len)):
            ...
            raw_logp = logits.log_softmax(dim=-1).squeeze(dim=0)
            logp = raw_logp.clone()

            if i < int(min_len):
                logp[self.audio_token_size] = torch.tensor(float('-inf'), dtype=self.dtype, device=logp.device)

            if one_step_only:
                top_k = min(prob_top_k, logp.numel())
                top_logprobs, top_token_ids = torch.topk(logp.float(), k=top_k)
                raw_top_logprobs, raw_top_token_ids = torch.topk(raw_logp.float(), k=top_k)

                payload = {
                    "step": i,
                    "audio_token_size": self.audio_token_size,
                    "top_token_ids": top_token_ids.cpu(),
                    "top_logprobs": top_logprobs.cpu(),
                    "raw_top_token_ids": raw_top_token_ids.cpu(),
                    "raw_top_logprobs": raw_top_logprobs.cpu(),
                }

                if save_full_prob:
                    payload["logprobs"] = logp.float().cpu()
                    payload["raw_logprobs"] = raw_logp.float().cpu()
                    payload["probs"] = logp.float().softmax(dim=-1).cpu()

                if prob_dump_path is not None:
                    import os
                    os.makedirs(os.path.dirname(prob_dump_path), exist_ok=True)
                    torch.save(payload, prob_dump_path)
